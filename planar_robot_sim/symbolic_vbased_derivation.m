...
Xe = sym('xe_', [3 1]);
Xe(3) = sym(0);
H_times_J_frameshifted = simplify(transpose(J)*(H_pos -Xe));
...
%simplify(subs(H_times_J_frameshifted, Q(1:num_links), 

V = sym('v',[num_links 1]); 
Qmax = sym('qmax', [num_links 1]);
Qmin = sym('qmin', [num_links 1]);
Q_eq = sym('q_eq', [num_links 1]);
Cond = sym('cond', [num_links 1]);
for i = 1:num_links
    Q_eq(i) = solve(eq(Q(i), Qmin(i) + (Qmax(i) - Qmin(i))/sym(2)*sin(V(i))), Q(i));
    Cond(i) = in(Qmin(i) + sin(V(i))*(Qmax(i)/2 - Qmin(i)/2), 'real');
end
% planar_vmax = 
% KUKA_Qmax(1) = 
assume(Cond(1:num_links));
Vbased_H_times_J_frameshifted = subs(H_times_J_frameshifted, Q(1:num_links), transpose(Q_eq(1:num_links)));

V_ctrlr = (Qmax(1:num_links) - Qmin(1:num_links))/sym(2).*sin(V(1:num_links)).*Vbased_H_times_J_frameshifted;
V_ctrlr_planar = subs(V_ctrlr, [Alpha(1:num_links) D(1:num_links)],zeros(1,num_links*2));
