...
%   default "0" angle.
    ...
    for i = 1:qdim
        L(i) = Link('d', D(i), 'a', A(i), 'alpha', Alpha(i), 'standard', 'qlim', [Qmin(i) Qmax(i)]);
    end
    robot = SerialLink(L);
    if nargin == 6
        robot.plotopt = plotopt;
    else
        robot.plotopt = {'noshadow','nojaxes', 'nowrist','noname','linkcolor',0.7*[1,1,1], 'ortho','noshading','notiles','jointcolor',0.6*[1,1,1]};
    end
end
